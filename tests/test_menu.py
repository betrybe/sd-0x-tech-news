...
NEW_NOTICE_ANALYZER = {'url': 'https://www.tecmundo.com.br/vamos.htm',
                       'title': 'Vamoscomtudo',
                       'timestamp': '2020-11-23T11:00:01',
                       'writer': 'Leonardo',
                       'shares_count': 1,
...
def test_validar_saida_do_console_collector_menu(capsys):
    ...
    assert "Selecione uma das opções a seguir:\n 1 - Importar notícias a partir de um arquivo CSV;\n 2 - Exportar notícias para CSV;\n 3 - Raspar notícias online;\n 4 - Sair." in out
...
def test_executar_opcao_importar_collector_menu(capsys):
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.csv_importer") as mock_importer, patch("tech_news.menu.create_news") as create_news:
...
def test_executar_opcao_exportar_collector_menu(capsys):
    ...
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.csv_exporter") as mock_exporter:
...
def test_executar_opcao_raspar_collector_menu(capsys):
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.scrape") as scraper, patch("tech_news.menu.create_news") as create_news:
...
def test_executar_opcao_titulo_do_console_analyzer_menu(capsys):
    ...
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.search_by_title") as mock_search_by_title:
...
def test_executar_opcao_data_do_console_analyzer_menu(capsys):
    db.news.delete_many({})
    db.news.insert_one(NEW_NOTICE_ANALYZER)
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.search_by_date") as mock_search_by_date:
...
def test_executar_opcao_fonte_do_console_analyzer_menu(capsys):
    db.news.delete_many({})
    db.news.insert_one(NEW_NOTICE_ANALYZER)
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.search_by_source") as mock_search_by_source:
...
def test_executar_opcao_categoria_do_console_analyzer_menu(capsys):
    db.news.delete_many({})
    db.news.insert_one(NEW_NOTICE_ANALYZER)
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.search_by_category") as mock_search_by_category:
...
def test_executar_opcao_top5_noticias_do_console_analyzer_menu(capsys):
    ...
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.top_5_news") as mock_top_5_news:
        mocked_input.side_effect = ["5", ""]
        analyzer_menu()
...
def test_executar_opcao_top5_categorias_do_console_analyzer_menu(capsys):
    ...
    db.news.insert_many([NEW_NOTICE_1, NEW_NOTICE_2, NEW_NOTICE_3,
                         NEW_NOTICE_4, NEW_NOTICE_5, NEW_NOTICE_6])
    with patch("builtins.input") as mocked_input, patch("tech_news.menu.top_5_categories") as mock_top_5_categories:
